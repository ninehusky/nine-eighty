...
public class EITest {
    @Test
    @DisplayName("EI enables interrupts")
    public void enablesInterrupts() {
        ...
        EI.enableInterrupts().execute(regs, flags, bus);

        assertEquals(true, regs.isInterruptsEnabled());
    }

    @Test
    @DisplayName("EI runs for correct number of cycles, length")
    public void lengthAndCycles() {
        ...
        assertEquals(1, EI.enableInterrupts().execute(regs, flags, bus));
